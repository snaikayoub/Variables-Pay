...
#[Route('/api/responsable/division/prime-fonction')]
#[IsGranted('ROLE_RESPONSABLE_DIVISION')]
final class ResponsableDivisionPrimeFonctionController extends AbstractController
{
    #[Route('', name: 'api_responsable_division_prime_fonction_list', methods: ['GET'])]
    public function list(Request $request, PrimeFonctionRepository $repo): JsonResponse
    {
        ...
        $status = $request->query->getString('status', PrimeFonction::STATUS_SERVICE_VALIDATED);
        if (!in_array($status, [
    ...
    #[Route('/batch/validate', name: 'api_responsable_division_prime_fonction_batch_validate', methods: ['POST'])]
    public function batchValidate(
        ...
        $pfs = $repo->findByIdsForDivisionValidator($user, $ids);
        $pfsById = [];
        foreach ($pfs as $pf) {
            $pfsById[$pf->getId()] = $pf;
        }
    ...
    #[Route('/batch/retour', name: 'api_responsable_division_prime_fonction_batch_retour', methods: ['POST'])]
