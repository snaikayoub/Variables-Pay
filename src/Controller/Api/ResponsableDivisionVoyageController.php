...
#[Route('/api/responsable/division/voyages')]
#[IsGranted('ROLE_RESPONSABLE_DIVISION')]
final class ResponsableDivisionVoyageController extends AbstractController
{
    #[Route('', name: 'api_responsable_division_voyages_list', methods: ['GET'])]
    public function list(Request $request, VoyageDeplacementRepository $repo): JsonResponse
    {
        ...
        $status = $request->query->getString('status', VoyageDeplacement::STATUS_SERVICE_VALIDATED);
        if (!in_array($status, [
        ...
        $items = $repo->findByDivisionValidatorAndStatusAndType($user, $status, $typePaie);
    ...
    #[Route('/batch/validate', name: 'api_responsable_division_voyages_batch_validate', methods: ['POST'])]
    public function batchValidate(
        ...
        $voyages = $repo->findByIdsForDivisionValidator($user, $ids);
        $byId = [];
        ...
        foreach ($ids as $id) {
            ...
            if (!$voyage_deplacement->can($v, 'division_validate')) {
                $skipped[] = ['id' => $id, 'reason' => 'Transition not allowed'];
                continue;
            }

            $voyage_deplacement->apply($v, 'division_validate');
    ...
    #[Route('/batch/reject', name: 'api_responsable_division_voyages_batch_reject', methods: ['POST'])]
    ...
    #[Route('/batch/retour', name: 'api_responsable_division_voyages_batch_retour', methods: ['POST'])]
    public function batchRetourService(
        Request $request,
        VoyageDeplacementRepository $repo,
        WorkflowInterface $voyage_deplacement,
        EntityManagerInterface $em
        ...
        foreach ($ids as $id) {
            ...
            if (!$voyage_deplacement->can($v, 'retour_service')) {
                $skipped[] = ['id' => $id, 'reason' => 'Transition not allowed'];
                continue;
            }

            $voyage_deplacement->apply($v, 'retour_service');
            $v->setUpdatedAt(new \DateTimeImmutable());
            $returned[] = $id;
        }
