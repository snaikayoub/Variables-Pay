...
#[Route('/api/responsable/division/prime-performance')]
#[IsGranted('ROLE_RESPONSABLE_DIVISION')]
final class ResponsableDivisionPrimePerformanceController extends AbstractController
{
    #[Route('', name: 'api_responsable_division_prime_performance_list', methods: ['GET'])]
    public function list(Request $request, PrimePerformanceRepository $repo): JsonResponse
    {
        ...
        $status = $request->query->getString('status', PrimePerformance::STATUS_SERVICE_VALIDATED);
        if (!in_array($status, [
        ...
        $items = $repo->findByDivisionAndStatusAndType($user, $status, $typePaie);
    ...
    #[Route('/batch/validate', name: 'api_responsable_division_prime_performance_batch_validate', methods: ['POST'])]
    public function batchValidate(
        ...
        $pps = $repo->findByIdsForDivisionValidator($user, $ids);
        $ppsById = [];
        foreach ($pps as $pp) {
            $ppsById[$pp->getId()] = $pp;
        }
    ...
    #[Route('/batch/retour', name: 'api_responsable_division_prime_performance_batch_retour', methods: ['POST'])]
